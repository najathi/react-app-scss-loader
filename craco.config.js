    ...
    return {
        ...
        webpack: {
            ...
            configure: (webpackConfig, { env, paths }) => {
                ...
                const dex = `${paths.appNodeModules}${path.sep}@dex`;

                webpackConfig.module.rules = webpackConfig.module.rules.map(rule => {

                    let tsString = "file.ts";

                    if (rule.include && typeof rule.test === "object" && tsString.search(rule.test)) rule.include = [rule.include, dex];

                    if (rule.oneOf) {

                        rule.oneOf = rule.oneOf.map(one => {

                            if (one.include && typeof one.test === "object" && tsString.search(one.test)) one.include = [one.include, dex];
